<template>
  <div class="controller">
    <div class="name">Контролл 3</div>
    <div class="input-wrapper">
      <div class="label" v-show="!$store.state.contoller2Opened" @click="openController2();$refs.controll.focus()"><span>{{$store.state.contoller2 | numbers}}</span></div>
      <input type="number" min="0" ref="controll"
             @keypress="isNumber"
             v-model.number="input"
             @blur="changeController1($store.state.contoller2)"
             @keyup.esc="closeController2(); changeController2($store.state.contoller1); $event.target.blur()"
             @keyup.enter="changeController2($refs.controll.value); changeController1($refs.controll.value); closeController2(); $event.target.blur()"
             @keydown.tab="changeController2($refs.controll.value); changeController1($refs.controll.value); closeController2(); $event.target.blur()"
             @focus="openController2(); $event.target.value = 0; $event.target.select()" class="input">
    </div>
  </div>
</template>
...
<script>
  ...
  computed: {
    input: {
      get () {
        return this.$store.state.contoller2
      },
      set (value) {
        this.changeController1(value);
      }
    }
  },
  methods: {
